...
export default function ProfileScreen() {
  ...
  return (
    <div className="screen">
      ...
      <div className="card">
        <div style={{ fontSize: 16, fontWeight: 700 }}>@{user?.username || user?.user_id}</div>
        <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 2 }}>📱 {user?.phone || 'No phone on file'}</div>
        {profile && <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 4 }}>Joined {profile.joined?.slice(0, 10)}</div>}
      </div>

      <button className="btn btn-success btn-block" onClick={claimBonus}>🎁 Claim Daily Bonus</button>
      ...
      {referral && (
        <div className="card">
          ...
          <div style={{ fontSize: 13, marginTop: 6 }}>Referrals so far: <b>{referral.referral_count}</b></div>
          <button className="btn btn-secondary btn-block" style={{ marginTop: 10 }} onClick={copyReferralLink}>
            📋 Copy referral link
          </button>
        </div>
      )}
      ...
      <div className="card">
        ...
        {!transactions && <div style={{ color: 'var(--text-dim)', fontSize: 13 }}>Loading…</div>}
        {transactions?.length === 0 && <div style={{ color: 'var(--text-dim)', fontSize: 13 }}>No transactions yet.</div>}
        {transactions?.map((tx, i) => (
          <div key={i} className="row" style={{ padding: '8px 0', borderBottom: i < transactions.length - 1 ? '1px solid var(--border)' : 'none' }}>
            <span style={{ fontSize: 13 }}>{TX_ICON[tx.type] || '•'} {tx.type.replace(/_/g, ' ')}</span>
            <span style={{ fontWeight: 700, color: tx.amount >= 0 ? 'var(--green-bright)' : 'var(--danger)', fontSize: 13 }}>
