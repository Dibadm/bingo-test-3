...
export default function LiveGameScreen({
  ...
  const toggleAuto = async () => {
    ...
    setState(prev => prev ? { ...prev, auto_win: next } : prev);
  };
    ...
    <div className="screen">
      ...
      <div className="card" style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>
          Call {state.call_count}/{state.max_calls}
        </div>
        {state.last_call && (
...
function NumberGrid({ called }: { called: number[] }) {
          ...
          return (
            <div
              ...
              style={{
                ...
                background: hit ? 'var(--gold)' : 'var(--bg-elevated)',
                color: hit ? '#1a1306' : 'var(--text-faint)',
...
function CardView({ card }: { card: GameCard }) {
  const calledSet = new Set(card.called);
  return (
    <div className="card">
      <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 8 }}>
        Cartela #{card.card_number}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 4 }}>
        {LETTERS.map(l => (
          <div key={l} style={{ textAlign: 'center', fontSize: 11, fontWeight: 700, color: 'var(--gold-bright)' }}>
            {l}
          </div>
        ))}
        {[0, 1, 2, 3, 4].map(row =>
...
function ResultScreen({ state, onDone }: { state: GameState; onDone: () => void }) {
      ...
      >
        ...
        {!won && state.winners.length > 0 && (
          <div style={{ fontSize: 13, color: 'var(--text-dim)', marginTop: 6 }}>
            {state.winners.length} winner{state.winners.length > 1 ? 's' : ''} —{' '}
            {fmt(state.per_winner_amount)} ETB each
          </div>
        )}
      </div>
